Polarity - Linux Beta README
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

This document contains information about the Polarity Linux
...
Hi!  This is a BETA release of the Polarity client for Linux.
...
Minimum requirements:
    ...
    * Computer Processor: 3rd gen Intel® Core™ i5 or better / AMD FX-4100 Quad-Core Processor
      (recommended: Intel® Core™ i7-5820K / AMD FX-8350)
    * Computer Memory: 4096MB (recommended: 8192MB or more)
    * Linux Operating System: A reasonably modern Linux environment
          is required.  If you are running a 64-bit Linux distribution then
          you may need its 32-bit compatibility environment installed.
    * PulseAudio or ALSA Linux system sound software.  A recent PulseAudio
      ...
      information.
    * Video/Graphics Card:
          o NVIDIA GTX 670 or equivalent, or better
          (nVidia cards are recommended for Polarity)
      ...
      **NOTE**: Second Life absolutely requires you to have recent, correctly-
      ...
      TROUBLESHOOTING section if you encounter problems starting Polarity.
...
The Polarity Linux client can entirely run from the directory you have
...
Run ./polarity from the installation directory to start the client.

For in-world MOVIE and MUSIC PLAYBACK, you will need GStreamer 0.10
...
User data is stored in the hidden directory ~/.polarity by default; you may
...
run from.  Most of this is also replicated in ~/.polarity/logs/SecondLife.log
...
PROBLEM 1:- Polarity fails to start up, with a warning on the console like:
...
PROBLEM 2:- My whole system seems to hang when running Polarity.
SOLUTION:- This is typically a hardware/driver issue.  The first thing to
   ...
   graphics card (see PROBLEM 1, then 7).
SOLUTION:- Some residents with ATI cards have reported that running
   'sudo aticonfig --locked-userpages=off' before running Polarity solves
   their stability issues.
SOLUTION:- As a last resort, you can disable most of Polarity's advanced
   graphics features by editing the 'secondlife' script and removing the '#'
   from the line which reads '#export LL_GL_NOEXT=x'

PROBLEM 3:- After I minimize the Polarity window, it's just blank when
   it comes back.
SOLUTION:- Some Linux desktop 'Visual Effects' features are incompatible
   with Polarity.  One reported solution is to use your desktop
...
SOLUTION:- The most common solution is to ensure that you have the 'esd'
   ...
   start Polarity.  Users of Ubuntu (and some other) Linux systems can
...
PROBLEM 7: Degraded performance compared to previously acceptable snowglobe(1.x)
  based client.
SOLUTION:-
    ,^~~~-.         .-~~~"-.
   :  .--. \       /  .--.  \
...
The 'polarity' script which launches Polarity contains some
...
* AUDIO - Edit the 'secondlife' script and you will see these audio
  ...
  LL_BAD_FMOD_ALSA.  Polarity tries to use OpenAL, ESD, OSS, then ALSA
...
We're pleased to have released the Polarity client's source code under
...
The Polarity Issue Tracker:
<https://polarityviewer.atlassian.net/>
This is the right place for finding known issues and reporting new
bugs in all Polarity releases if you find that the Troubleshooting
