...
bool KCWindlightInterface::applySkySettings(const LLSD& settings)
{
	...
	if (settings.has("sky"))
	{
		...
		LL_WARNS("KCWindlightInterface") << "Agent is not within a defined zone. Trying default now" << LL_ENDL;
	}
		...
		//TODO: improved data processing, possibly just use LLSD as input instead
			...
			if(boost::regex_search(data, match2, Parcel_exp))
			{
				...
				if (match2[1]=="Parcel")
				{
					...
					LLNotifications::instance().add("PVWindLight_Parcel_Ask", args, payload, boost::bind(&KCWindlightInterface::callbackParcelWL, this, _1, _2));
...
bool KCWindlightInterface::loadFromParcel(LLParcel *parcel)
{
	...
	if (parseParcelForWLSettings(parcel->getDesc(), payload))
	{
		...
		else
		{
			...
			mSetWLNotification = LLNotifications::instance().add("PVWindLight_Parcel_Ask", args, payload, boost::bind(&KCWindlightInterface::callbackParcelWL, this, _1, _2));
		}
		return true;
	}
...
void KCWindlightInterface::onClickWLStatusButton()
{
	...
	if (mWLset)
	{
 		...
 		if (parcel)
		{
			...
			mClearWLNotification = LLNotifications::instance().add("PVWindLight_Parcel_Clear", args, payload, boost::bind(&KCWindlightInterface::callbackParcelWLClear, this, _1, _2));
		}
	}
}
...
{
	mWLset = pwl_status;
	//gStatusBar->updateParcelIcons(); // <polarity> Currently causes crash, we don't use statusbar for now.
}
