...
// <xenhat> Avatar-Physics-Patch
static constexpr F32 PHYSICS_FPS = 30.f;
static constexpr F32 time_iteration_step = 1.f / PHYSICS_FPS;
// </xenhat> Avatar-Physics-Patch
...
{
	LLVector3 dir_world = mMotionDirectionVec * (mJointState->getJoint()->getWorldRotation());
	dir_world.normalize();
...
{
	static const F32 world_to_model_scale = 100.f;
	return toLocal(((mJointState->getJoint()->getWorldPosition()) - mPosition_world) * world_to_model_scale);
}
...
F32 LLPhysicsMotion::calculateAcceleration_local(const F32 velocity_local)
{
	...
	static const F32 equationpart1 = 1.f / smoothing;
	static const F32 equationpart2 = smoothing - 1.f;
	const F32 acceleration_local = (velocity_local - mVelocityJoint_local)
		* equationpart1
		+ mAccelerationJoint_local
		* (equationpart2 / smoothing);

	return acceleration_local;
}
...
BOOL LLPhysicsMotion::onUpdate(F32 time)
{
	...
	if (time_delta <= time_iteration_step) // <xenhat/> Avatar-Physics-Patch
	{
		return FALSE;
	}
	...
	static LLCachedControl<bool> physics_test(gSavedSettings, "AvatarPhysicsTest", false);
	const F32 behavior_maxeffect = !physics_test ? getParamValue(MAX_EFFECT) : 1.0f;
...
// <xenhat/> Avatar-Physics-Patch
		...
		if ((behavior_maxeffect < FLT_EPSILON) && fabs(position_current_local - position_user_local) < FLT_EPSILON)
		{
		...
		if (behavior_maxeffect < FLT_EPSILON)
		...
		/*
		// Write out debugging info into a spreadsheet.4
		LL_WARNS() << llformat("\nposition_new_local:       %f\nvelocity_new_local:       %f\nacceleration_new_local:   %f\n\ntime_delta:               %f\n\nmPosition_world[0]:       %f\nmPosition_world[1]:       %f\nmPosition_world[2]:       %f\n\nforce_net:                %f\nforce_spring:             %f\nforce_accel:              %f\nforce_damping:            %f\nforce_drag:               %f\n\nspring_length:            %f\nvelocity_joint_local:     %f\nacceleration_joint_local: %f\n",
			position_new_local,
			velocity_new_local,
