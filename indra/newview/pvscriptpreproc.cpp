...
////////////////////////////////////////////////////////////////////////////////////////
/// Turn parts of the code off
#define ENABLE_TEXT_COMPRESS false

////////////////////////////////////////////////////////////////////////////////////////
/// questionable things to remove:
...
//TODO: we can use boost regex for this, probably - Xenhat
...
void FSLSLPreprocessor::cache_script(const std::string &name, std::string content)
{
	...
	LLFILE* infile = LLFile::fopen(path.c_str(), "wb");
	if
		(infile)
	{
		fwrite(content.data(), content.size(), 1, infile);
	}
	fflush(infile);
}

std::string FSLSLPreprocessor::scopeript2(std::string& top, S32 fstart, char left, char right)
{
...
void FSLSLPreprocessor::shredder(std::string& text)
{
	S32 cursor = 0;
	if (text.empty())
	{
...
std::string FSLSLPreprocessor::reformat_lazy_lists(std::string script)
{
...
std::string FSLSLPreprocessor::quicklabel()
{
...
*/
std::string FSLSLPreprocessor::reformat_switch_statements(std::string script)
{
	std::string buffer = script;
	{
		try
		{
			// optimization to skip over blocks of whitespace faster
	...
	if (item && self)
	{
		...
		if (result == LL_ERR_NOERR)
		{
			...
			if (boost::filesystem::native(name))
			{
				...
				static LLCachedControl<bool> workaround_crash(gSavedSettings, "PVDebug_WorkaroundPreprocCrash", false);
				if (self->caching_files.size() <= 0 && workaround_crash)
				{
					LL_WARNS("FSLSLPreprocessor") << "Attempted to cache and compile a script without the proper preparation; aborted." << LL_ENDL;
					return;
				}
				llassert(self->caching_files.size() <= 0);
...
bool FSLSLPreprocessor::trace_include_files::found_include_directive(ContextT const& ctx, std::string const &filename, bool include_next)
{
...
template <typename ContextT>
void FSLSLPreprocessor::trace_include_files::opened_include_file(ContextT const& ctx,
...
template <typename ContextT>
void FSLSLPreprocessor::trace_include_files::returning_from_include_file(ContextT const& ctx)
{
	ContextT& usefulctx = const_cast<ContextT&>(ctx);
...
void FSLSLPreprocessor::trace_include_files::throw_exception(ContextT const& ctx, ExceptionT const& e)
{
	std::string err;
