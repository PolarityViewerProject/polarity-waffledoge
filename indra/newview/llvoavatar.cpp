	...
	if (updateIsFullyLoaded()) // changed?
	{
		...
		else
		{
			...
			else
			{
				...
				LLColor4 cloud_color = gPVOldAPI->getColor(getID(), LLColor4(1, 1, 1, 0.5f));
...
LLColor4 LLVOAvatar::getNameTagColor(const LLUUID& av_id)
{
	...
	return gPVOldAPI->getColor(av_id, color_name);
}
...
//			//BD - Freeze World
			static LLCachedControl<bool> freeze_frame(gSavedSettings, "PVRender_FreezeWorld");
...
BOOL LLVOAvatar::isTextureVisible(LLAvatarAppearanceDefines::ETextureIndex type, U32 index) const
{
	...
	else
	{
		...
		return ((isTextureDefined(type) || isSelf())
			&& (((getTEImage(type)->getID() != IMG_INVISIBLE) || (LLPipeline::sShadowRender && (PVRender_ShadowDetailRigged == 2)))
				|| LLDrawPoolAlpha::sShowDebugAlpha));
	}
}
